...
class DataBalancer:
    ...
    def balance_training_set(self, df, ratio=1.0, balance=True, save=True):
        ...
        if save:
            df.to_csv("data/balanced_train_data.csv", index=False)
        return df


class DataSplitter:

    def __init__(self, val_size=0.1, test_size=0.2, random_state=42):
    ...
    def split(self, path, save_splits=True):
        ...
        df_train, df_val = train_test_split(
            df_,
            test_size=self.val_size if self.val_size > 0 else None, 
            stratify=df_['TARGET'],
            random_state=self.random_state
        )
        ...
        if save_splits:
            ...
            df_val.to_csv('data/split_data_val.csv', index=False)
