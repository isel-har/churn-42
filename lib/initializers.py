...
class Initializers:
    ...
    def he_init(cls, shape:tuple):
        ...
        return cls.rng.random(size=shape, dtype=np.float64) * std_dev
    

    @classmethod
    def random_init(cls, shape:tuple):
        return cls.rng.random(size=shape, dtype=np.float64)
    ...
    def constant_init(val, shape:tuple):
        return np.full(shape=shape, fill_value=val, dtype=np.float64)
